const advancedResults = (model, populate) => async(req,res,next) => {
    let query;
		...
		//Pagination result
		const pagination = {};
		if(endIndex<total){//if the last index of the current page is less than the total number of items, it means we have a next page
			pagination.next = {
				page:page+1,
				limit
			}
		...
		if(startIndex>0){//if the first index of the current page is bigger 0 that means we have a previous page
			pagination.prev={
				page:page-1,
				limit
			}
